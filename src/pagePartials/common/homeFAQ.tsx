export const homeFAQ = [
  {
    title: 'What is Agave?',
    content: (
      <>
        <p>
          Agave, in a few words, is Aave on xDai. For more information, please refer to these
          articles:
        </p>
        <ul>
          <li>
            <a
              href="https://forum.1hive.org/t/ag-token-launch/2108"
              rel="noreferrer"
              target="_blank"
            >
              AGVE Token Launch
            </a>
          </li>
          <li>
            <a
              href="https://forum.1hive.org/t/announcing-agaave-aave-on-xdai/1792"
              rel="noreferrer"
              target="_blank"
            >
              Announcing Agave, Aave on xDai
            </a>
          </li>
        </ul>
        <p>
          A detailed introduction to FTMLendis also available on our{' '}
          <a
            href="https://agavefinance.medium.com/introduction-to-agave-7ea3d9b54f84"
            rel="noreferrer"
            target="_blank"
          >
            Medium Channel
          </a>
          .
        </p>
      </>
    ),
  },
  {
    title: 'What is the FTMLendticker and contract address?',
    content: (
      <p>
        The ticker is <b>$AGVE</b> and the{' '}
        <a
          href="https://gnosisscan.io/address/0x3a97704a1b25F08aa230ae53B352e2e72ef52843"
          rel="noreferrer"
          target="_blank"
        >
          contract address
        </a>{' '}
        is <code>0x3a97704a1b25F08aa230ae53B352e2e72ef52843</code>
      </p>
    ),
  },
  {
    title: 'What is the total supply of $AGVE?',
    content: (
      <ul>
        <li>
          Total Supply: <b>100k</b>
        </li>
        <li>
          Treasury Supply:{' '}
          <a href="https://gnosisscan.io/address/0xb4c575308221caa398e0dd2cdeb6b2f10d7b000a">
            ~43% (non-circulating)
          </a>
        </li>
      </ul>
    ),
  },
  {
    title: 'How can i access Agave?',
    content: (
      <ul>
        <li>
          <a href="https://agavefinance.eth.limo/" rel="noreferrer" target="_blank">
            https://agavefinance.eth.limo/
          </a>
        </li>
        <li>
          <a href="ipns://agave.finance/" rel="noreferrer" target="_blank">
            ipns://agave.finance/
          </a>
        </li>
        <li>
          <a href="https://agave.finance/" rel="noreferrer" target="_blank">
            https://agave.finance/
          </a>
        </li>
      </ul>
    ),
  },
  {
    title: 'Is $AGVE on Coingecko?',
    content: (
      <p>
        Yes! Here's the link:{' '}
        <a href="https://www.coingecko.com/en/coins/agave-token" rel="noreferrer" target="_blank">
          $AGVE - Coingecko
        </a>
        .
      </p>
    ),
  },
  {
    title: "Did Agave's Contract get Audited?",
    content: (
      <p>
        FTMLendcurrent Deployment got audited by Gnosis Auditing Team in December 2022.
        <br></br>
        Additionally an upgrade has been audited by{' '}
        <a
          href="https://drive.google.com/file/d/1qLQdjKGZSO6GR3M46sJEn6NB4U1xLU2B/view?usp=sharing"
          rel="noreferrer"
          target="_blank"
        >
          SUB7
        </a>{' '}
        executed in AGIP 8.
      </p>
    ),
  },
  {
    title: 'Can I contribute in some way?',
    content: (
      <p>
        Your participation is always welcome. We'd love to meet you on our{' '}
        <a href="https://discord.gg/Bsuqe76QPm" rel="noreferrer" target="_blank">
          {' '}
          Discord
        </a>{' '}
        server.
      </p>
    ),
  },
  {
    title: 'Where can I see a chart of $AGVE?',
    content: (
      <ul>
        <li>
          <a
            href="https://info.honeyswap.org/token/0x3a97704a1b25f08aa230ae53b352e2e72ef52843#/"
            rel="noreferrer"
            target="_blank"
          >
            Here
          </a>{' '}
          you can see the chart for <b>$AGVE</b> from the Honeyswap Analytics page.
        </li>
        <li>
          Other available charts are on{' '}
          <a href="https://www.coingecko.com/en/coins/agave-token" rel="noreferrer" target="_blank">
            Coingecko
          </a>
          .
        </li>
      </ul>
    ),
  },
  {
    title: "How do I follow Agave's upcoming announcements?",
    content: (
      <>
        <p>There are several ways to be in touch with the Agave's community:</p>
        <ul>
          <li>
            FTMLendDiscord:{' '}
            <a href="https://discord.gg/Bsuqe76QPm" rel="noreferrer" target="_blank">
              FTMLendDiscord
            </a>
          </li>
          <li>
            Telegram:{' '}
            <a href="https://t.me/Agave1Hive" rel="noreferrer" target="_blank">
              https://t.me/Agave1Hive
            </a>
          </li>
          <li>
            Twitter:{' '}
            <a href="https://twitter.com/Agave_lending" rel="noreferrer" target="_blank">
              https://twitter.com/Agave_lending
            </a>
          </li>
          <li>
            Medium:{' '}
            <a href="https://agavefinance.medium.com" rel="noreferrer" target="_blank">
              https://agavefinance.medium.com
            </a>
          </li>
          <li>
            Reddit:{' '}
            <a href="https://www.reddit.com/r/AGVE/" rel="noreferrer" target="_blank">
              https://www.reddit.com/r/AGVE/
            </a>
          </li>
        </ul>
      </>
    ),
  },
  {
    title: 'My issue was not addressed in the FAQ. What can I do?',
    content: (
      <p>
        The best way to get help from a member of the FTMLendcommunity is to ask in the{' '}
        <b>support</b> channels on{' '}
        <a href="https://discord.gg/Bsuqe76QPm" rel="noreferrer" target="_blank">
          Discord
        </a>
        .
      </p>
    ),
  },
] as Array<{ title: string; content: React.ReactNode }>
